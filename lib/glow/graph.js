;(function () {
    ...
    function graph(options) {
		...
		this.graph_options = { series: { shadowSize:0 }, crosshair: { mode: "xy", color: "rgba(0,0,0,1)" }, xaxes: [{position: 'bottom',
		axisLabel: options.xtitle, 
		axisLabelUseCanvas: true, axisLabelFontSizePixels: 12, axisLabelFontFamily: 'Verdana, Arial', axisLabelPadding: 10 }],yaxes: [{position: 'left',
		axisLabel: options.ytitle, 
		axisLabelUseCanvas: true, axisLabelFontSizePixels: 12, axisLabelFontFamily: 'Verdana, Arial', axisLabelPadding: 10 }]}
        this.__width = 640
        this.__height = 400
        this.__title = ''
        this.__plot = null
    ...
    function vp_graph(options) {
		...
		if (title.length > 0) t1 = title + '<br>&nbsp'	
    	//if (ytitle.length > 0) {
    	//	t2 = ytitle
    	//	if (xtitle.length > 0) t2 += ' VERSUS. '+xtitle
		...
		if (t1.length > 0) options.title = t1	
		//Make options.xtitle and options.ytitle
	    	var t3 = ''
	    	var t4 = ''
			if (ytitle.length > 0) t3 = ytitle
	    	if (xtitle.length > 0) t4 = xtitle
	    	 
	    	if (t3.length > 0) options.ytitle = t3
	    	if (t4.length > 0) options.xtitle = t4
	    //	else if (t1.length > 0) options.title = t1
		//// End options.xtitle and options.ytitle
